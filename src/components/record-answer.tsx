...
export const RecordAnswer = ({
    ...
    if (isRecording) {
      ...
      if (finalAnswer.length < 30) {
        toast.error("Answer too short", {
          description: "Please speak more than 30 characters.",
        });
        return;
      }
  ...
  const generateResult = async (
    ...
    const prompt = `
      ...
      User Answer: "${userAns}"
      Correct Answer: "${qstAns}"
      Please compare the user's answer to the correct answer, and provide a rating (1-10) and feedback for improvement.
      Return result in JSON format with keys "ratings" (number) and "feedback" (string).
    `;
    ...
    try {
      ...
      console.error("AI Result Parse Error:", error);
      return { ratings: 0, feedback: "Failed to generate feedback." };
    } finally {
      setIsAiGenerating(false);
    }
  };
  ...
  const saveUserAnswer = async () => {
    ...
    try {
      ...
      if (!querySnap.empty) {
        toast.info("Already Answered", {
          description: "You’ve already submitted this answer.",
        });
        return;
      }
      ...
      toast.success("Saved", { description: "Answer saved successfully." });
      setUserAnswer("");
      stopSpeechToText();
    } catch (error) {
